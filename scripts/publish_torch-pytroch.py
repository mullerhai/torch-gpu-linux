...
ARTIFACT_ID = "pytorch"
VERSION = "2.13.0-1.5.14-GA-1.10"
CLASSIFIER = "linux-x86_64-gpu"
...
def stage_artifact(stage: Path, source_jar: Path, version: str | None = None) -> list[Path]:
    ...
    # Main jar (copy from source, filename includes classifier)
    main_out = out_dir / f"{ARTIFACT_ID}-{ver}-{CLASSIFIER}.jar"
...
def bundle_all(stage: Path, bundle_dir: Path, version: str | None = None) -> Path:
    ...
    zip_path = bundle_dir / f"pytorch-{ver}-{CLASSIFIER}-{stamp}.zip"
    if zip_path.exists():
        zip_path.unlink()
